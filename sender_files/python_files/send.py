...
def configure_packet(lcd_lock): # TODO Rewrite with selectable layers
     ...
     with lcd_lock:
            ...
            layers = (ip, src_port, dst_port, dstMAC)
            packet = generate_UDP_packet(layers)
...
def generate_UDP_packet(layers):
    ip = layers[0]
    src_port = layers[1]
    dst_port = layers[2]
    dstMAC = layers [3]
...
def update_statistics_loop():
     ...
     while True:
            ...
            bw, bw_unit = shared_functions.calculate_bandwidth(bw)
...
if __name__ == '__main__':
     ...
     while True:
            if lcd.is_pressed(LCD.SELECT): # Configure packet
                 ...
                 print delay_seconds
                 SEASIDE.send_SEASIDE(c_socket, SEASIDE_FLAGS.DELAY, delay_seconds)
                 ...
                 SEASIDE.send_SEASIDE(c_socket, SEASIDE_FLAGS.DELAY, [0]) 
